...
import static com.shijingsh.ai.jsat.utils.SystemInfo.L2CacheSize;
import static com.shijingsh.ai.jsat.utils.SystemInfo.LogicalCores;
...
import com.shijingsh.ai.jsat.utils.FakeExecutor;
import com.shijingsh.ai.jsat.utils.FakeExecutor;
...
public abstract class GenericMatrix extends Matrix {
     ...
     */
    protected static int NB2 = (int) sqrt(SystemInfo.L2CacheSize / (8.0 * 2.0));
    ...
    public void mutableAdd(final double c, final Matrix b, ExecutorService threadPool) {
        ...
        for (int threadId = 0; threadId < SystemInfo.LogicalCores; threadId++) {
            ...
            threadPool.submit(new Runnable() {
                ...
                public void run() {
                    for (int i = 0 + ID; i < rows(); i += SystemInfo.LogicalCores)
                        for (int j = 0; j < cols(); j++)
                            increment(i, j, c * b.get(i, j));
                    latch.countDown();
                }
    ...
    public void multiplyTranspose(final Matrix b, final Matrix C, ExecutorService threadPool) {
        ...
        final int jLimit = b.rows();
        final int kLimit = this.cols();
        final int blockStep = Math.min(NB2, Math.max(iLimit / SystemInfo.LogicalCores, 1));// reduce block size so we can use all cores if needed.
        final CountDownLatch cdl = new CountDownLatch(SystemInfo.LogicalCores);

        for (int threadNum = 0; threadNum < SystemInfo.LogicalCores; threadNum++) {
    ...
    public void multiply(final Matrix b, final Matrix C, ExecutorService threadPool) {
        ...
        if (this.rows() / NB2 >= SystemInfo.LogicalCores)// Perform block execution only when we have a large enough matrix to keep ever
                                              // core busy!
        {
    ...
    public void mutableMultiply(final double c, ExecutorService threadPool) {
        ...
        for (int threadID = 0; threadID < SystemInfo.LogicalCores; threadID++) {
            ...
            threadPool.submit(new Runnable() {
                ...
                public void run() {
                    for (int i = ID; i < rows(); i += SystemInfo.LogicalCores)
                        for (int j = 0; j < cols(); j++)
                            set(i, j, get(i, j) * c);
                    latch.countDown();
                }
    ...
    public Matrix[] lup(ExecutorService threadPool) {
        ...
        try {
            List<Future<Integer>> bigIndecies = new ArrayList<Future<Integer>>(SystemInfo.LogicalCores);
                ...
                for (int threadNumber = 0; threadNumber < SystemInfo.LogicalCores; threadNumber++) {
                    ...
                    bigIndecies.add(threadPool.submit(new Callable<Integer>() {
                        ...
                        public Integer call() throws Exception {
                            ...
                            int largestIndex = -1;
                            for (int i = kk + 1 + threadID; i < UU.rows(); i += SystemInfo.LogicalCores) {
