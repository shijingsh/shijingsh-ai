...
import static com.shijingsh.ai.jsat.linear.VecPaired.extractTrueVec;
...
public class RTree<V extends Vec> implements IncrementalCollection<V> {
    ...
    public void search(Vec query, double range, IntList neighbors, DoubleList distances) {
        ...
        while (nIter.hasNext()) {
            ...
            if ((dist = dm.dist(query, VecPaired.extractTrueVec(get(indx)))) <= range)
                dIter.set(dist);
            ...
            if (minDistN <= curBest.last().getProbability()) {
                if (N.isLeaf()) {
                    for (int indx : N.points) {
                        double dist = dm.dist(query, VecPaired.extractTrueVec(get(indx)));
    ...
    private RNode cloneChangeContext(RNode toClone) {
        if (toClone != null)
            if (toClone instanceof RTree.RNode)
